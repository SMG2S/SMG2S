...
pVecMap::pVecMap(MPI_Comm ncomm, S lbound, S ubound)
{
...
S Loc2Glob(S local_index){
	if((local_index < global_size) && (local_index >= 0)){return lower_bound + local_index;}
	else {return -1;}
}
template<typename T, typename S> 
S Glob2Loc(S global_index){
        if((global_index >= local_size) && (global_index < upper_bound)){return global_index - lower_bound;}
}

                //get
template<typename T, typename S> 
int pVecMap::GetOwner(S index)
{
